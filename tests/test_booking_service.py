...
def test_update_draft_sets_fields():
    token = _token()
    bid = svc.create_draft("chat1", "7700", token)["data"]["booking_id"]
    res = svc.update_draft(bid, date="2026-07-01", time_start="10:00", time_end="11:00", field=2)
    assert res["ok"]
...
def test_update_draft_rejected_after_payment():
    ...
    res = svc.update_draft(bid, players=99)
    assert not res["ok"]
...
def test_request_payment_slot_taken():
    ...
    r1 = svc.request_payment(b1, t1)
    r2 = svc.request_payment(b2, t2)
    assert r1["ok"]
    assert not r2["ok"]
...
def test_submit_payment_proof_confirms():
    ...
    bid = _ready_draft(token)
    svc.request_payment(bid, token)
    res = svc.submit_payment_proof(bid, proof_media_id="media123")
    assert res["ok"]
...
def test_submit_payment_proof_wrong_state():
    token = _token()
    bid = svc.create_draft("chat1", "7700", token)["data"]["booking_id"]
    res = svc.submit_payment_proof(bid)
    assert not res["ok"]
    assert res["code"] == "BOOKING_WRONG_STATE"
...
def test_reject_payment_allows_retry():
    ...
    svc.reject_payment(bid, "amount too low",
    ...
    ok = svc.submit_payment_proof(bid, parsed={"bank": "kaspi", "amount": 35000,
...
def test_get_expired_bookings_selects_expired_only():
    ...
    svc.request_payment(b_expired, t1)
    ...
    svc.request_payment(b_fresh, t2)
